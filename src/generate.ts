...
function inferResourceType(target: string): string | null {
  const t = target.toLowerCase().replace(/[^a-z0-9]/g, '_');
  return TARGET_TO_RESOURCE_TYPE[t] ?? null;
}
...
function detectWorkflowTargets(projectRoot: string): {
  ...
  let entries: Dirent<string>[];
  try { entries = readdirSync(workflowsDir, { withFileTypes: true }); } catch { return { workflows, targets }; }
  for (const entry of entries) {
...
export async function generateSpec(
  ...
  const allResources = new Map<string, string>();   // name → resourceType
  ...
  for (const comp of components) {
    ...
    for (const fact of facts) {
      ...
      if (resourceType && !allResources.has(resourceName)) {
        allResources.set(resourceName, resourceType);
      }
    }
  ...
  lines.push(
    ...
    `// Project: ${projectName}`,
    `// Components: ${components.length} | Infrastructure nodes: ${allInfraNodes.size} | Resources: ${allResources.size} | Contracts: ${contracts.length}`,
  ...
  if (workflowInfo.workflows.length > 0) {
    lines.push(emitNode('github_actions', 'ci_runner', 'trusted'));
  }
  lines.push('');
